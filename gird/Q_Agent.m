...
% value weight
agent_struct.Wc = 0.0 * (randn(N_hidden+1, 1) - 0.5)*2; % value function
agent_struct.Wa = 0.0 * (rand(N_hidden+1, agent_struct.N_action) - 0.5)*2; % action
...
mu = softmax(agent_struct.Wa' * [1;relu(agent_struct.Wh' * s)],1);

% discrete
if rand < agent_struct.epsilon % random
    action = randi(agent_struct.N_action);
else % greedy
    [~, action] = max(mu);
end
...
if isTerminal == 0
    ...
    mu_dash = softmax(agent_struct.Wa' * hidden_dash, 1);
    a_dash = double(mu_dash==max(mu_dash));
